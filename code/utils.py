...
def evaluate(df, total):
    hitrate_5 = 0  # 这里分别初始化了不同召回数量（5、10、20、40、50）下的命中率和平均倒数排名指标为 0。这些指标将在后续的计算中逐步累加。
    mrr_5 = 0
    ...
    for _, g in tqdm(gg):  # 获取用户的正样本物品 ID
        try:
            item_id = g[g['label'] == 1]['article_id'].values[0]
        except Exception as e:
            continue

        predictions = g['article_id'].values.tolist()  # 获取用户的推荐物品列表

        rank = 0 # 计算正样本物品在推荐列表中的排名
        while predictions[rank] != item_id:
            rank += 1

        if rank < 5:  # 根据排名更新不同召回数量下的评估指标.
            # 命中率（Hit Rate）：如果正样本物品的排名小于某个召回数量（如 5、10、20 等），则说明在该召回数量下推荐命中了用户的正样本，相应的命中率指标加 1。
            # 平均倒数排名（MRR）：MRR 的计算公式为 1/(rank+1)，其中 rank 是正样本物品在推荐列表中的排名。
            mrr_5 += 1.0 / (rank + 1)
            hitrate_5 += 1
